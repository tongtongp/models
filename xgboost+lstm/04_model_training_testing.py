...
def train_model(
    ...
    with torch.no_grad():
        train_delta_scaled = model(train_seq_tensor, train_tab_tensor).cpu().numpy()
    train_delta = fe.unscale_targets(train_delta_scaled, scalers)
    train_lstm_values = np.clip(train_prev + output_alphas * train_delta, 0.0, 1.0)
    train_residuals = train_actual - train_lstm_values
    ...
    for output_index in range(len(targets)):
        ...
        reg.fit(train_xgb_features[valid_mask], train_residuals[valid_mask, output_index])
...
def predict_row(bundle: dict[str, Any], row: dict[str, Any], previous_targets: np.ndarray) -> np.ndarray:
    ...
    seq = row["weather_seq_28"].astype(np.float32)
    ...
    if xgb_models and len(xgb_models) == len(bundle["targets"]):
        ...
        for output_index, model in enumerate(xgb_models):
            if model is None:
                xgb_corrections.append(0.0)
            else:
                xgb_corrections.append(float(model.predict(xgb_features)[0]))
        values = lstm_values + np.asarray(xgb_corrections, dtype=np.float32)
    else:
